...
*/
;(function ($) {
    $.fn.xx_easyUpload = function (opts) {
        /*var defaults = {
        ...
        var option = $.extend({}, $.fn.xx_easyUpload.defaults, opts);
        // 通用函数集合
        var F = {
        ...
        this.each(function (index, element) {
            ...
            var easyManager = {
                init: function () {
                    var $html = '';
                    $html = '<div class="xx_easy_upload-container"><div class="xx_easy_upload-head">';
                    $html += '<input type="file" '
                    $html += option.multi ? 'multiple ' : '';
                    $html += 'class="xx_fileInput" data-count="0" style="display:none;" />';
                    $html += '<span class="xx_easy_upload_select xx_noselect">' + option.selectText + '</span>';
                    $html += option.multi ? '<span class="xx_easy_upload_head_btn1 xx_noselect">上传</span>' : '';
                    $html += option.multi ? '<span class="xx_easy_upload_head_btn2 xx_noselect">删除</span>' : '';
                    $html += option.multi ? '<i class="xx_easyUploadIcon xx_noselect xx_head_check" data-checked="no">&#xe693;</i>' : '';
                    $html += option.showNote ? '<span class="xx_easy_upload_note">' + option.note + '</span>' : '';
                    $html += '</div>';
                    $html += '<ul class="xx_easy_upload_queue"></ul>';
                    $html += '</div>';
                    _ele.html($html);
                    this.bindHead();
                },
                bindHead: function () {
                    ...
                    $('.xx_easy_upload_select').off('click').click(function () {
                        $(this).parent().find('.xx_fileInput').trigger('click');
                    });
                    $('.xx_fileInput').off('change').change(function () {
                        ...
                        $(this).parent().find('.xx_head_check').html('&#xe693;').attr('data-checked', 'no');
                    });
                    $('.xx_head_check').off('click').click(function () {
                    ...
                    $('.xx_easy_upload_head_btn1').off('click').click(function () {
                        var queueUl = $(this).parent().parent().find('.xx_easy_upload_queue');
                    ...
                    $('.xx_easy_upload_head_btn2').off('click').click(function () {
                        var queueUl = $(this).parent().parent().find('.xx_easy_upload_queue');
                ...
                bindQueue: function () {
                    var _this = this;
                    $('.xx_queue_check').off('click').click(function () {
                        ...
                        var hItem = $(this).parent().parent().parent().parent().find('.xx_head_check');
                        if (checkedAll) {
                    ...
                    $('.xx_easy_upload_upbtn').off('click').click(function () {
                    ...
                    $('.xx_easy_upload_delbtn').off('click').click(function () {
                        var upStatus = $(this).parent().parent().find('.xx_queue_check').attr('data-up');
                        if (upStatus != '3') {
                ...
                _renderFile: function (fileArr, target) {
                    var queueUl = $(target).parent().parent().find('.xx_easy_upload_queue');
                    ...
                    function render(file) {
                        ...
                        if (fileType == 'bmp' || fileType == 'jpg' || fileType == 'jpeg' || fileType == 'png' || fileType == 'gif') {
                            ...
                            preview = '<img class="xx_easy_upload_img" src="' + imgSrc + '" />';
                        } else if (fileType == 'rar' || fileType == 'zip' || fileType == 'arj' || fileType == 'z') {
                            preview = '<i class="xx_easy_upload_icon xx_easyUploadIcon">&#xe69d;</i>';
                        } else {
                            preview = '<i class="xx_easy_upload_icon xx_easyUploadIcon">&#xe64d;</i>';
                        }
                        var sHtml = '';
                        sHtml += '<p class="xx_status xx_status1">可以上传</p>';
                        sHtml += '<p class="xx_status xx_status2">等待上传</p>';
                        sHtml += '<p class="xx_status xx_status3">上传中</p>';
                        sHtml += '<p class="xx_status xx_status4">上传失败</p>';
                        sHtml += '<p class="xx_status xx_status5">上传成功</p>';
                        var $html = '';
                        $html += '<li class="xx_easy_upload_queue_item" data-index="' + file.index + '">';
                        $html += option.showPreview ? '<div class="xx_easy_upload_preview xx_queue_item-section">' + preview + '</div>' : '';
                        $html += '<div class="xx_easy_upload_file1 xx_queue_item-section">';
                        $html += '<p class="xx_easy_upload_filename">' + f.name + '</p>';
                        $html += '<p class="xx_easy_upload_progress">';
                        $html += '<span class="xx_easy_upload_bar"></span>';
                        $html += '</p>';
                        $html += '</div>';
                        $html += '<div class="xx_easy_upload_file2 xx_queue_item-section">';
                        $html += '<p class="xx_easy_upload_fiesize">' + F.formatFileSize(f.size) + '</p>';
                        $html += '<p class="easy_upload_percent">0%</p>';
                        $html += '</div>';
                        $html += '<div class="xx_easy_upload_status xx_queue_item-section">';
                        $html += file.allow ? sHtml : '<p class="xx_status xx_status6">文件不允许</p>';
                        $html += '</div>';
                        $html += '<div class="xx_easy_upload_btn xx_queue_item-section">';
                        $html += file.allow ? '<p class="xx_easy_upload_upbtn xx_btn xx_noselect">上传</p>' : '';
                        $html += '<p class="xx_easy_upload_delbtn xx_btn xx_noselect">删除</p>';
                        $html += '</div>';
                        $html += '<div class="xx_easy_upload_checkone xx_queue_item-section">';
                        $html += option.multi ? '<i class=' + file.allow + '"xx_easyUploadIcon xx_noselect xx_queue_check xx_queue_check_allow_" data-checked="no" data-up="1">&#xe693;</i>' : '';
                        $html += '</div>';
                        if (option.multi) {
                            $(queueUl).append($html);
                        } else {
                    ...
                    for (var i = 0; i < fileArr.length; i++) {
                        if (option.multi) {
                            var qItemNum = $(queueUl).find('.xx_easy_upload_queue_item:visible').length;
                ...
                _handleCheck: function (opt) {
                    if (opt.type == 'all') {
                        if (opt.check == 'yes') {
                            ...
                            var qItems = $(opt.target).parent().parent().find('.xx_queue_check');
                ...
                _countCheck: function (target) {
                    var checkedAll = true;
                    var qItems = $(target).parent().parent().parent().find('.xx_queue_check');
                    for (var i = 0; i < qItems.length; i++) {
                        ...
                        if (allowNewPost) {
                            ...
                            $.ajax({
                                ...
                                success: function (res) {
                                    ...
                                    // if (res.code!=option.okCode){ //不校验上传图片情况
                                    //   allowNewPost = true;
                                    //   if (option.multi) {
                                    ...
                                    // if (option.multi) { //不管是不是多图片,都放到返回
                                    response.success.push(res);
                                    response.successSingle=[res]; //显示对应的上传文件
                ...
                _setUpStatus: function (opt, type) {
                    ...
                    if (type == 1) {
                        $(param.ele).find('.xx_queue_check').attr('data-up', 3);
                    } else {
                        $(param.ele).find('.xx_queue_check').attr('data-up', 4);
                    }
                },
                _setStatus2: function (target) {
                    ...
                    allowFiles.forEach(function (item) {
                        ...
                        if (qItem.upStatus == '1') {
                            $(qItem.statusDiv).find('.xx_status').hide().end().find('.xx_status2').show();
                            $(qItem.ele).find('.xx_easy_upload_upbtn').hide();
                            $(qItem.ele).find('.xx_queue_check').attr('data-up', 2);
                        }
                    });
                },
                _showProgress: function (index, target) {
                    ...
                    $(param.ele).find('.xx_easy_upload_upbtn').hide(400);
                    $(param.statusDiv).find('.xx_status').hide().end().find('.xx_status3').show();
                    ...
                    showTimer = setInterval(function () {
                        if (loadedPercent < 100) {
                            ...
                            $(upPeacent).text('100%');
                            $(upBar).css("width", "100%");
                            $(param.statusDiv).find('.xx_status').hide().end().find('.xx_status5').show();
                            upFiniehed = true;
                            allowNewPost = true;
                ...
                _findEle: function (index, target) {
                    var obj = {};
                    obj.ele = $(target).find('.xx_easy_upload_queue_item[data-index=' + index + ']');
                    obj.upBar = $(obj.ele).find('.xx_easy_upload_bar');
                    obj.upPeacent = $(obj.ele).find('.easy_upload_percent');
                    obj.statusDiv = $(obj.ele).find('.xx_easy_upload_status');
                    obj.upStatus = $(obj.ele).find('.xx_queue_check').attr('data-up');
                    return obj;
                },
                _findItems: function (type, target) {
                    var arr = [];
                    if (type == 1) {
                        var icon = $(target).find('.xx_queue_check_allow_true[data-up="1"][data-checked="yes"]:visible');
                    } else {
                        var icon = $(target).find('.xx_queue_check[data-up="1"][data-checked="yes"]:visible,.xx_queue_check[data-up="2"][data-checked="yes"]:visible,.xx_queue_check[data-up="4"][data-checked="yes"]:visible');
                    }
                ...
                _deleFiles: function (arr, target) {
                    ...
                    arr.forEach(function (item) {
                        $(target).find('.xx_easy_upload_queue_item[data-index=' + item + ']').hide().find('.xx_queue_check').hide();
                ...
                _handleFailed: function (param) {
                    ...
                    $(param.statusDiv).find('.xx_status').hide().end().find('.xx_status4').show();
                },
