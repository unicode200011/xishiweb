...
public class UserAuthInfoController extends BaseController {
    ...
    public Object update(UserAuthInfo userAuthInfo) {
        UserAuthInfo dbData = userAuthInfoService.selectById(userAuthInfo.getId());
        //更新用户信息
        User user = userService.selectById(dbData.getUserId());
        Integer state = userAuthInfo.getState();
        user.setApplyStatus(state == 2?4:3);
        if(state == 2) {
            user.setShower(1);
        }
        user.setApplyStatusTime(new Date());
        userService.updateById(user);
        if(state == 2){
            //添加主播信息
            Live live = new Live();
            String roomId = this.getRoomId();
            live.setUserId(dbData.getUserId());
            live.setRoomNum(roomId);
            live.setStreamName(roomId);
    ...
    public String getRoomId(){
        String roomNum = ToolUtil.getRandomNum(9);
        int room_num = liveService.selectCount(new EntityWrapper<Live>().eq("room_num", roomNum));
        while (room_num > 0) {
            roomNum = ToolUtil.getRandomNum(9);
            room_num = liveService.selectCount(new EntityWrapper<Live>().eq("room_num", roomNum));
        }
        return roomNum;
    }
