...
MgrUser.openAddMgr = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/mgr/admin_add'
    });
...
MgrUser.openChangeUser = function () {
    if (this.check()) {
        ...
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/mgr/admin_edit/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
MgrUser.delMgrUser = function () {
    if (this.check()) {
        ...
        Feng.confirm("是否删除管理员" + MgrUser.seItem.account + "?",operation);
    }
};
...
MgrUser.freezeAccount = function () {
    if (this.check()) {
        if(this.seItem.roleid == "超级管理员"){
            Feng.error("超级管理员不能被冻结")
            return
        }
...
MgrUser.unfreeze = function () {
    if (this.check()) {
        if(this.seItem.roleid == "超级管理员"){
            Feng.error("超级管理员没有被冻结")
            return
        }
        var adminId = this.seItem.id;
