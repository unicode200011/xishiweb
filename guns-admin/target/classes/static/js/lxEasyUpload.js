...
*/
;(function ($) {
    $.fn.lx_easyUpload = function (opts) {
        /*var defaults = {
        ...
        var option = $.extend({}, $.fn.lx_easyUpload.defaults, opts);
        // 通用函数集合
        var F = {
        ...
        this.each(function (index, element) {
            ...
            var easyManager = {
                init: function () {
                    var $html = '';
                    $html = '<div class="lx_easy_upload-container"><div class="lx_easy_upload-head">';
                    $html += '<input type="file" '
                    $html += option.multi ? 'multiple ' : '';
                    $html += 'class="lx_fileInput" data-count="0" style="display:none;" />';
                    $html += '<span class="lx_easy_upload_select lx_noselect">' + option.selectText + '</span>';
                    $html += option.multi ? '<span class="lx_easy_upload_head_btn1 lx_noselect">上传</span>' : '';
                    $html += option.multi ? '<span class="lx_easy_upload_head_btn2 lx_noselect">删除</span>' : '';
                    $html += option.multi ? '<i class="lx_easyUploadIcon lx_noselect lx_head_check" data-checked="no">&#xe693;</i>' : '';
                    $html += option.showNote ? '<span class="lx_easy_upload_note">' + option.note + '</span>' : '';
                    $html += '</div>';
                    $html += '<ul class="lx_easy_upload_queue"></ul>';
                    $html += '</div>';
                    _ele.html($html);
                    this.bindHead();
                },
                bindHead: function () {
                    ...
                    $('.lx_easy_upload_select').off('click').click(function () {
                        $(this).parent().find('.lx_fileInput').trigger('click');
                    });
                    $('.lx_fileInput').off('change').change(function () {
                        ...
                        $(this).parent().find('.lx_head_check').html('&#xe693;').attr('data-checked', 'no');
                    });
                    $('.lx_head_check').off('click').click(function () {
                    ...
                    $('.lx_easy_upload_head_btn1').off('click').click(function () {
                        var queueUl = $(this).parent().parent().find('.lx_easy_upload_queue');
                    ...
                    $('.lx_easy_upload_head_btn2').off('click').click(function () {
                        var queueUl = $(this).parent().parent().find('.lx_easy_upload_queue');
                ...
                bindQueue: function () {
                    var _this = this;
                    $('.lx_queue_check').off('click').click(function () {
                        ...
                        var hItem = $(this).parent().parent().parent().parent().find('.lx_head_check');
                        if (checkedAll) {
                    ...
                    $('.lx_easy_upload_upbtn').off('click').click(function () {
                    ...
                    $('.lx_easy_upload_delbtn').off('click').click(function () {
                        var upStatus = $(this).parent().parent().find('.lx_queue_check').attr('data-up');
                        if (upStatus != '3') {
                ...
                _renderFile: function (fileArr, target) {
                    var queueUl = $(target).parent().parent().find('.lx_easy_upload_queue');
                    ...
                    function render(file) {
                        ...
                        if (fileType == 'bmp' || fileType == 'jpg' || fileType == 'jpeg' || fileType == 'png' || fileType == 'gif') {
                            ...
                            preview = '<img class="lx_easy_upload_img" src="' + imgSrc + '" />';
                        } else if (fileType == 'rar' || fileType == 'zip' || fileType == 'arj' || fileType == 'z') {
                            preview = '<i class="lx_easy_upload_icon lx_easyUploadIcon">&#xe69d;</i>';
                        } else {
                            preview = '<i class="lx_easy_upload_icon lx_easyUploadIcon">&#xe64d;</i>';
                        }
                        var sHtml = '';
                        sHtml += '<p class="lx_status lx_status1">可以上传</p>';
                        sHtml += '<p class="lx_status lx_status2">等待上传</p>';
                        sHtml += '<p class="lx_status lx_status3">上传中</p>';
                        sHtml += '<p class="lx_status lx_status4">上传失败</p>';
                        sHtml += '<p class="lx_status lx_status5">上传成功</p>';
                        var $html = '';
                        $html += '<li class="lx_easy_upload_queue_item" data-index="' + file.index + '">';
                        $html += option.showPreview ? '<div class="lx_easy_upload_preview lx_queue_item-section">' + preview + '</div>' : '';
                        $html += '<div class="lx_easy_upload_file1 lx_queue_item-section">';
                        $html += '<p class="lx_easy_upload_filename">' + f.name + '</p>';
                        $html += '<p class="lx_easy_upload_progress">';
                        $html += '<span class="lx_easy_upload_bar"></span>';
                        $html += '</p>';
                        $html += '</div>';
                        $html += '<div class="lx_easy_upload_file2 lx_queue_item-section">';
                        $html += '<p class="lx_easy_upload_fiesize">' + F.formatFileSize(f.size) + '</p>';
                        $html += '<p class="easy_upload_percent">0%</p>';
                        $html += '</div>';
                        $html += '<div class="lx_easy_upload_status lx_queue_item-section">';
                        $html += file.allow ? sHtml : '<p class="lx_status lx_status6">文件不允许</p>';
                        $html += '</div>';
                        $html += '<div class="lx_easy_upload_btn lx_queue_item-section">';
                        $html += file.allow ? '<p class="lx_easy_upload_upbtn lx_btn lx_noselect">上传</p>' : '';
                        $html += '<p class="lx_easy_upload_delbtn lx_btn lx_noselect">删除</p>';
                        $html += '</div>';
                        $html += '<div class="lx_easy_upload_checkone lx_queue_item-section">';
                        $html += option.multi ? '<i class=' + file.allow + '"lx_easyUploadIcon lx_noselect lx_queue_check lx_queue_check_allow_" data-checked="no" data-up="1">&#xe693;</i>' : '';
                        $html += '</div>';
                        if (option.multi) {
                            $(queueUl).append($html);
                        } else {
                    ...
                    for (var i = 0; i < fileArr.length; i++) {
                        if (option.multi) {
                            var qItemNum = $(queueUl).find('.lx_easy_upload_queue_item:visible').length;
                ...
                _handleCheck: function (opt) {
                    if (opt.type == 'all') {
                        if (opt.check == 'yes') {
                            ...
                            var qItems = $(opt.target).parent().parent().find('.lx_queue_check');
                ...
                _countCheck: function (target) {
                    var checkedAll = true;
                    var qItems = $(target).parent().parent().parent().find('.lx_queue_check');
                    for (var i = 0; i < qItems.length; i++) {
                ...
                _setUpStatus: function (opt, type) {
                    ...
                    if (type == 1) {
                        $(param.ele).find('.lx_queue_check').attr('data-up', 3);
                    } else {
                        $(param.ele).find('.lx_queue_check').attr('data-up', 4);
                    }
                },
                _setStatus2: function (target) {
                    ...
                    allowFiles.forEach(function (item) {
                        ...
                        if (qItem.upStatus == '1') {
                            $(qItem.statusDiv).find('.lx_status').hide().end().find('.lx_status2').show();
                            $(qItem.ele).find('.lx_easy_upload_upbtn').hide();
                            $(qItem.ele).find('.lx_queue_check').attr('data-up', 2);
                        }
                    });
                },
                _showProgress: function (index, target) {
                    ...
                    $(param.ele).find('.lx_easy_upload_upbtn').hide(400);
                    $(param.statusDiv).find('.lx_status').hide().end().find('.lx_status3').show();
                    ...
                    showTimer = setInterval(function () {
                        if (loadedPercent < 100) {
                            ...
                            $(upPeacent).text('100%');
                            $(upBar).css("width", "100%");
                            $(param.statusDiv).find('.lx_status').hide().end().find('.lx_status5').show();
                            upFiniehed = true;
                            allowNewPost = true;
                ...
                _findEle: function (index, target) {
                    var obj = {};
                    obj.ele = $(target).find('.lx_easy_upload_queue_item[data-index=' + index + ']');
                    obj.upBar = $(obj.ele).find('.lx_easy_upload_bar');
                    obj.upPeacent = $(obj.ele).find('.easy_upload_percent');
                    obj.statusDiv = $(obj.ele).find('.lx_easy_upload_status');
                    obj.upStatus = $(obj.ele).find('.lx_queue_check').attr('data-up');
                    return obj;
                },
                _findItems: function (type, target) {
                    var arr = [];
                    if (type == 1) {
                        var icon = $(target).find('.lx_queue_check_allow_true[data-up="1"][data-checked="yes"]:visible');
                    } else {
                        var icon = $(target).find('.lx_queue_check[data-up="1"][data-checked="yes"]:visible,.lx_queue_check[data-up="2"][data-checked="yes"]:visible,.lx_queue_check[data-up="4"][data-checked="yes"]:visible');
                    }
                ...
                _deleFiles: function (arr, target) {
                    ...
                    arr.forEach(function (item) {
                        $(target).find('.lx_easy_upload_queue_item[data-index=' + item + ']').hide().find('.lx_queue_check').hide();
                ...
                _handleFailed: function (param) {
                    ...
                    $(param.statusDiv).find('.lx_status').hide().end().find('.lx_status4').show();
                },
