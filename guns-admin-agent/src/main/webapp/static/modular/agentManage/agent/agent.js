...
Agent.initColumn = function () {
    return [
        ...
        {title: '累计收益（西施币）', field: 'showerNum', visible: true, align: 'center', valign: 'middle'},
...
Agent.openAgentDetail = function () {
    var agentId = $("#agentId").val()
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/agent/agent_update_account/' + agentId
        });
        this.layerIndex = index;
};
...
Agent.openWithdrawal = function () {
    ...
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/agent/openWithdrawal/' + userId
    });
    this.layerIndex = index;
};
