...
; (function ($) {
    ...
    var defaults = {
      allowFileTypes: '*.pdf;*.doc;*.docx;*.jpg;*.mp3;*.MP3',//允许上传文件类型，格式'*.pdf;*.doc;'
      ...
      dynamicAllowedFileTypesFunc: null, //文件选择后回调函数
      multiNumFunc: null, // 图片上传数量
      multiFunc: null // 动态文件上传
    };
    ...
    this.each(function (index, element) {
      ...
      var easyManager = {
        init: function () {
          ...
          $html += option.multi ? '<span class="easy_upload_head_btn1 noselect easy-more-upload-class">上传</span>' : '';
          $html += option.multi ? '<span class="easy_upload_head_btn2 noselect easy-more-upload-class">删除</span>' : '';
          $html += option.multi ? '<i class="easyUploadIcon noselect head_check easy-more-upload-class" data-checked="no">&#xe693;</i>' : '';
        ...
        _checkFile: function (fileArr, target) {
          var typeArr = F.getFileTypes(!!option.dynamicAllowedFileTypesFunc ? option.dynamicAllowedFileTypesFunc() : option.allowFileTypes);
          if (typeArr.length > 0) {
        ...
        _renderFile: function (fileArr, target) {
          ...
          function render(file) {
            ...
            $html += '</div>';
            $html += '<div class="easy_upload_checkone queue_item-section easy-more-upload-class">';
          ...
          for (var i = 0; i < fileArr.length; i++) {
            if (option.multi) {
              ...
              var tempMutiNum = !!option.multiNumFunc ? option.multiNumFunc() : option.multiNum;
              if (qItemNum<tempMutiNum) render(fileArr[i]);
            } else {
              render(fileArr[i]);
            }
          }
        ...
        _deleFiles: function(arr,target) {
          ...
          function dele(item) {
            response.success.forEach(function(item1,index1){
              response.del=[item1]; //这里是返回被删除的元素;
          ...
          arr.forEach(function(item){
            ...
            dele(item);
            // var qItem = _this._findEle(item, target);  //满足当前业务需要注释掉
