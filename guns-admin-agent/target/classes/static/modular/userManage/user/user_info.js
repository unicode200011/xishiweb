 ...
 * 提交设置抽成
 */
UserInfoDlg.submitRate = function() {
var bootstrapValidator = $('#validatorForm').data('bootstrapValidator');
    //手动触发验证
    bootstrapValidator.validate();
    if (bootstrapValidator.isValid()) {
        ...
        //提交信息
        var ajax = new $ax(Feng.ctxPath + "/user/submitRate", function(data){
            Feng.success("添加成功!");
            window.parent.User.table.refresh();
            UserInfoDlg.close();
        },function(data){
 ...
 * 提交审核用户申请
 */
UserInfoDlg.auditUserApply = function(state) {
var bootstrapValidator = $('#validatorForm').data('bootstrapValidator');
    //手动触发验证
    bootstrapValidator.validate();
    if (bootstrapValidator.isValid()) {
        ...
        if(!inviteId){
            Feng.error("该数据有误,请联系管理员")
            return
        }
        //提交信息
        var ajax = new $ax(Feng.ctxPath + "/user/auditUserApply", function(data){
            Feng.success("添加成功!");
            window.parent.User.table.refresh();
            UserInfoDlg.close();
        },function(data){
